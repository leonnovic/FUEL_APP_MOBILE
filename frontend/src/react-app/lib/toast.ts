...
function notifyListeners() { listeners.forEach(l => l([...toasts])); }

function getContainer(): HTMLDivElement {
  if (toastContainer) return toastContainer;
  const div = document.createElement('div');
  div.id = 'fuelpro-toast-container';
  div.style.cssText = 'position:fixed;top:16px;right:16px;z-index:99999;display:flex;flex-direction:column;gap:8px;max-width:380px;pointer-events:none;';
...
function renderToast(t: ToastMessage): HTMLDivElement {
  ...
  el.id = `toast-${t.id}`;
  el.style.cssText = 'pointer-events:auto;padding:12px 16px;border-radius:10px;font-size:13px;font-weight:500;display:flex;align-items:center;gap:10px;animation:toastSlideIn 0.3s ease;box-shadow:0 4px 20px rgba(0,0,0,0.3);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.08);';
...
export function toast(message: string, type: ToastType = 'info', duration = 3000): void {
  ...
  const t: ToastMessage = { id, message, type, duration, createdAt: Date.now() };
  toasts.push(t);
  notifyListeners();
...
export const toastError = (msg: string, dur?: number) => toast(msg, 'error', dur);
export const toastWarning = (msg: string, dur?: number) => toast(msg, 'warning', dur);
export const toastInfo = (msg: string, dur?: number) => toast(msg, 'info', dur);

/** Subscribe to toast changes (for React components) */
export function subscribeToToasts(cb: (toasts: ToastMessage[]) => void): () => void {
  listeners.push(cb);
