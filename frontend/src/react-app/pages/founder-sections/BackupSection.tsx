...
import { Database, Download, Upload, RefreshCw, Clock, CheckCircle2, AlertTriangle, HardDrive, Calendar } from 'lucide-react';
...
export default function BackupSection({ logAudit }: BackupSectionProps) {
  ...
  const [lastBackup, setLastBackup] = useState<string | null>(() => localStorage.getItem('fuelpro_last_backup'));
  ...
  const handleBackup = () => {
    ...
    setTimeout(() => {
      ...
      setLastBackup(now);
      setBackingUp(false);
      logAudit('Backup Completed', `Backup exported: ${(blob.size / 1024).toFixed(1)} KB`, 'success');
    }, 1200);
  };
  ...
  const handleRestore = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!confirm(`Restore ${Object.keys(data).length} keys? This will overwrite current data.`)) {
          setRestoring(false);
          return;
        }
        ...
        logAudit('Restore Completed', `Restored ${Object.keys(data).length} keys`, 'success');
        import('@/react-app/lib/app-reloader').then(({triggerSoftReload}) => triggerSoftReload(500));
      } catch {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-medium text-white flex items-center gap-2"><Database size={18} className="text-blue-400" /> Backup & Restore</h2>
          <p className="text-xs text-gray-500 mt-0.5">Export and import all FuelPro data</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-[#161618] border border-white/[0.06] rounded-xl p-5">
          ...
          <p className="text-xs text-gray-500 mb-4">Download all localStorage data as JSON</p>
          {lastBackup && (
            <div className="flex items-center gap-2 mb-3 text-[10px] text-gray-500">
              <Clock size={10} /> Last: {new Date(lastBackup).toLocaleString()} ({backupSize})
            </div>
          )}
          <button
          ...
          >
            {backingUp ? <><RefreshCw size={13} className="animate-spin" /> Creating...</> : <><Download size={13} /> Create Backup</>}
          </button>
        </div>
        ...
        <div className="bg-[#161618] border border-white/[0.06] rounded-xl p-5">
          ...
          <div className="mb-3 flex items-center gap-2">
            <input type="checkbox" checked={includeFiles} onChange={() => setIncludeFiles(!includeFiles)} className="rounded accent-amber-500" />
            <span className="text-[10px] text-gray-500">Include uploaded files</span>
          </div>
          <label className="w-full py-2.5 bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 text-xs rounded-lg border border-amber-500/20 transition-colors flex items-center justify-center gap-2 cursor-pointer">
            <Upload size={13} /> {restoring ? 'Restoring...' : 'Select Backup File'}
      ...
      {/* Storage Stats */}
      <div className="bg-[#161618] border border-white/[0.06] rounded-xl p-5">
        <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2"><HardDrive size={14} className="text-purple-400" /> Storage Breakdown</h3>
