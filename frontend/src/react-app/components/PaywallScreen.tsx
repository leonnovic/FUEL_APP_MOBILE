...
import {
  CreditCard, Clock, Check, Shield, Star, Key, Smartphone, ArrowLeft,
  Fuel, Zap, Globe, Tag, AlertTriangle, ChevronRight, Loader2, CheckCircle2,
  Timer, Lock, Gift
} from 'lucide-react';
import {
  checkTrialStatus, startTrial, checkTrialAbuse, recordTrialAbuse,
  createSubscription, recordPayment, applyCoupon, useCoupon,
  resolvePrice, resolveCountry, getAvailableGateways,
  loadTiers, loadRegionalPrices, type TierSlug, type GatewayInfo
...
export default function PaywallScreen({ onSubscriptionActive }: PaywallScreenProps) {
  ...
  const [step, setStep] = useState<'trial' | 'plans' | 'checkout' | 'success'>('trial');
  ...
  const getTierPrice = (tierId: TierSlug) => {
    ...
    if (!rp) return { amount: 0, currency: country.currency, gateways: [] as GatewayInfo[] };
  ...
  const discountedPrice = useMemo(() => {
    ...
    if (couponResult.discount < 100) {
      return Math.round(currentPrice.amount * (1 - couponResult.discount / 100));
    }
  ...
  const handlePayment = () => {
    ...
    setTimeout(() => {
      ...
      recordPayment(sub.id, selectedGateway || 'mpesa', discountedPrice, currentPrice.currency);
  ...
  const handleUnlockWithCode = () => {
    ...
    setIsProcessing(true);
    setTimeout(() => {
      const sub = createSubscription(user?.id || 'anonymous', 'lifetime', currentPrice.currency, 0, 'access-code', undefined);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-950 p-4 flex items-center justify-center">
        <div className="w-full max-w-lg">
          <button onClick={() => logout()} className="mb-4 text-sm text-gray-400 hover:text-white flex items-center gap-2 transition-colors">
          ...
          <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 shadow-2xl text-center">
            ...
            <h1 className="text-3xl font-bold text-white font-serif mb-2">{pwc('title', 'FuelPro')}</h1>
            <p className="text-gray-400 mb-6">{pwc('subtitle', 'Professional Fuel Management System')}</p>
            ...
            {trialStatus.active || trialStarted ? (
              <>
                <div className="bg-green-500/5 border border-green-500/20 rounded-xl p-6 mb-6">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <span className="text-green-400 font-medium">Trial Active</span>
                  </div>
                  <div className="text-5xl font-bold text-white font-mono mb-1">
                    {String(trialMinutes).padStart(2, '0')}:{String(trialSeconds).padStart(2, '0')}
                  </div>
                  <p className="text-sm text-gray-500">remaining in your free trial</p>
                </div>

                <div className="grid grid-cols-3 gap-3 mb-6">
                  {[{ icon: Fuel, label: 'Full Access' }, { icon: CreditCard, label: 'All Features' }, { icon: Shield, label: 'Secure' }].map(f => (
                    <div key={f.label} className="bg-white/5 rounded-lg p-3">
                      <f.icon size={18} className="mx-auto mb-1 text-amber-400" />
                      <p className="text-[10px] text-gray-400">{f.label}</p>
                    </div>
                  ))}
                </div>

                <button onClick={() => onSubscriptionActive()}
              ...
              <>
                <div className="rounded-xl p-6 mb-6" style={{ backgroundColor: pwc('accentColor', '#d97706') + '10', border: `1px solid ${pwc('accentColor', '#d97706')}30` }}>
                  <Timer size={32} className="mx-auto mb-3" style={{ color: pwc('accentColor', '#d97706') }} />
                  <h3 className="text-xl font-bold text-white mb-1">{pwc('trialHeadline', 'Free Trial')}</h3>
                  <p className="text-sm text-gray-400">{pwc('trialDescription', 'Experience the full power of FuelPro before you subscribe')}</p>
                </div>

                <div className="space-y-3 mb-6 text-left">
                  {(pwContent?.features || ['Complete fuel management', 'Real-time sales tracking', 'M-PESA integration', 'Invoice & receipt generation', 'Team management']).map((f: string, i: number) => (
                ...
                <button onClick={handleStartTrial}
                  className="w-full py-3 text-white font-semibold rounded-xl transition-all shadow-lg mb-3" style={{ background: `linear-gradient(to right, ${pwc('accentColor', '#d97706')}, ${pwc('accentColor', '#d97706')})` }}>
                ...
                {pwcB('showTrustBadges', true) && (
                  <div className="flex items-center justify-center gap-1 mt-4 text-[10px] text-gray-600">
                    <Shield size={10} /> {pwc('footerNote', 'No credit card required. One trial per device.')}
                  </div>
                )}
              </>
            )}
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-950 p-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white font-serif">Choose Your Plan</h1>
          ...
          <div className="grid grid-cols-5 gap-3 mb-8">
            {tiers.map(tier => {
              ...
              return (
                <button key={tier.id} onClick={() => { setSelectedTier(tier.id); setCouponResult(null); setCouponCode(''); }}
                  className={`relative bg-white/[0.03] border rounded-2xl p-5 text-left transition-all ${
                    isSelected ? 'border-amber-500/50 shadow-lg shadow-amber-500/10' : 'border-white/[0.06] hover:border-white/[0.12]'
                  }`}>
                  {tier.recommended && (
                    <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 text-[10px] px-2 py-0.5 bg-amber-500 text-white rounded-full font-medium">Best Value</span>
                  )}
                  <h3 className="text-sm font-semibold text-white mb-1">{tier.name}</h3>
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-2xl font-bold text-white">{tp.currency === 'USD' ? '$' : tp.currency}</span>
                    <span className="text-3xl font-bold text-amber-400">{tp.amount.toLocaleString()}</span>
                  </div>
                  <p className="text-[11px] text-gray-500 mb-3">{tier.durationDays === -1 ? 'One-time payment' : `Per ${tier.durationDays === 1 ? 'day' : tier.durationDays === 7 ? 'week' : tier.durationDays === 30 ? 'month' : 'year'}`}</p>
                  ...
                  {isSelected && <div className="mt-3 text-center text-[10px] text-amber-400 font-medium">Selected</div>}
                </button>
              );
            })}
          </div>
          ...
          <div className="max-w-md mx-auto mb-6">
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Tag size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
            ...
            {couponResult && (
              <div className={`mt-2 text-xs flex items-center gap-1 ${couponResult.valid ? 'text-emerald-400' : 'text-red-400'}`}>
                {couponResult.valid ? <CheckCircle2 size={12} /> : <AlertTriangle size={12} />}
          ...
          <div className="text-center">
            ...
            {trialStatus.active && (
              <button onClick={() => onSubscriptionActive()} className="mt-2 text-sm text-gray-500 hover:text-gray-300 transition-colors">
                Continue with trial
              </button>
            )}
          </div>
  ...
  if (step === 'checkout') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-950 p-4 flex items-center justify-center">
        <div className="w-full max-w-lg">
          <button onClick={() => setStep('plans')} className="mb-4 text-sm text-gray-400 hover:text-white flex items-center gap-2 transition-colors">
          ...
          <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 shadow-2xl">
            ...
            <p className="text-sm text-gray-500 mb-6">{country.name} — {country.currency}</p>

            {/* Order Summary */}
            <div className="bg-white/[0.02] rounded-xl p-4 mb-6 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">{selectedTierObj?.name} Plan</span>
                <span className="text-white">{currentPrice.currency} {currentPrice.amount.toLocaleString()}</span>
              </div>
              {couponResult?.valid && (
                <div className="flex justify-between text-sm">
                  <span className="text-purple-400 flex items-center gap-1"><Tag size={12} /> Coupon</span>
                  <span className="text-purple-400">-{couponResult.discount}{couponResult.discount > 100 ? '' : '%'}</span>
                </div>
              )}
              <div className="border-t border-white/[0.06] pt-2 flex justify-between">
                <span className="text-white font-medium">Total</span>
                <span className="text-amber-400 font-bold text-lg">{currentPrice.currency} {discountedPrice.toLocaleString()}</span>
              </div>
            </div>
                ...
                {currentPrice.gateways.map(g => (
                  <button key={g.id} onClick={() => setSelectedGateway(g.id)}
                    className={`p-3 rounded-xl border transition-all text-left ${
                      selectedGateway === g.id ? 'border-amber-500/50 bg-amber-500/5' : 'border-white/[0.06] hover:border-white/[0.12]'
                    }`}>
                    <div className="flex items-center gap-2">
                      <Zap size={14} className={selectedGateway === g.id ? 'text-amber-400' : 'text-gray-500'} />
            ...
            <div className="mb-6">
              <label className="text-xs text-gray-400 mb-2 block">Or enter access code</label>
            ...
            <button onClick={pwcB('enableAccessCode', true) && secretCode.trim() ? handleUnlockWithCode : handlePayment}
              disabled={isProcessing || ((pwcB('enableAccessCode', true) ? !secretCode.trim() : true) && !selectedGateway)}
              ...
              {isProcessing ? <><Loader2 size={18} className="animate-spin" /> Processing...</> : (pwcB('enableAccessCode', true) && secretCode.trim()) ? <><Key size={18} /> Unlock with Code</> : <><CreditCard size={18} /> Pay {currentPrice.currency} {discountedPrice.toLocaleString()}</>}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-950 p-4 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-400 mb-8">Your FuelPro {selectedTierObj?.name} plan is now active.</p>
        <div className="animate-pulse text-sm text-amber-400">Redirecting to dashboard...</div>
      </div>
    </div>
  );
}
