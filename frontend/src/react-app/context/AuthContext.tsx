...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const requestPasswordReset = useCallback(async (email: string): Promise<{ success: boolean; code?: string; message: string }> => {
    ...
    try {
      const r = await fetch(`${(import.meta as any).env?.VITE_REACT_APP_BACKEND_URL || ''}/api/auth/password-reset/request`, {
  ...
  const resetPassword = useCallback(async (email: string, newPassword: string): Promise<boolean> => {
    ...
    if (entry) {
      try {
        const r = await fetch(`${(import.meta as any).env?.VITE_REACT_APP_BACKEND_URL || ''}/api/auth/password-reset/confirm`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
