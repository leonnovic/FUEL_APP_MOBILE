...
export function useFounderAuth() {
  ...
  const checkSession = useCallback(() => {
    try {
      ...
      if (!raw) { setIsAuthenticated(false); setIsLoading(false); return; }
  ...
  const logout = useCallback(async () => {
    ...
    } catch { /* backend logout can fail silently */ }
    localStorage.removeItem(SESSION_KEY);
    setIsAuthenticated(false);
    setUsername('');
    setError('');
  ...
  const changePassword = useCallback(async (currentPw: string, newPw: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      const result = await trpc.founderAuth.changePassword.useMutation().mutateAsync({
