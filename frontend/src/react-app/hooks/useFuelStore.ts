...
interface FuelState {
  ...
  addNotification: (n: { title: string; message: string; severity: Notification['severity'] }) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (item) => {
        ...
        getEventBus().emit(Events.SALE_RECORDED, { item, cartSize: get().cart.length + 1 });
      },
      ...
      setPriceAlert: (alert) =>
        set(state => ({
          priceAlerts: [...state.priceAlerts.filter(a => a.fuelType !== alert.fuelType), alert],
        })),
      ...
      logAction: (action, detail) => {
        ...
        set(state => ({ recentActions: [entry, ...state.recentActions].slice(0, 50) }));
      },
    }),
    {
