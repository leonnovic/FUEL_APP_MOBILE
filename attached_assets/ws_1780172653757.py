"""Realtime WebSocket sync with SECURE cookie-based authentication.

SECURITY FIXES:
- JWT no longer passed in query string (was logged by proxies)
- Uses secure SameSite cookies for auth
...
async def ws_sync(websocket: WebSocket, ticket: str = Query(default="")):
    ...
    # Also accept cookie-based auth as fallback
    if not user_id:
        cookie_token = websocket.cookies.get("fuelpro_auth")
        user_id = _decode_jwt(cookie_token) if cookie_token else None
