...
export async function fetchExternalNews(countryCode: string = 'KE'): Promise<ExternalNewsItem[]> {
  const allNews: ExternalNewsItem[] = [];
  const sources = NEWS_SOURCES[countryCode] || NEWS_SOURCES['KE'];
...
// Auto-fetch wrapper
export async function autoFetchNews(countryCode: string = 'KE'): Promise<ExternalNewsItem[]> {
  if (!shouldFetchNews()) {
