...
const initialState: FuelState = {
  ...
  tabConfigurations: [
    ...
    { id: 'regional', label: 'Regional', originalLabel: 'Regional', description: 'Country-specific regulations & compliance', order: 23, visible: true },
...
export function FuelProvider({ children }: { children: ReactNode }) {
  ...
  const saveToStorage = () => {
    try {
      ...
      // Clean up old individual keys to free space
      const oldUserKey = user?.id ? `user_${user.id}_` : 'guest_';
      const keysToRemove = [
        'theme', 'themeSettings', 'userPreferences', 'companyData', 'signatures', 'invoiceCounter',
        'clients', 'invoices', 'debtHistory', 'salesHistory', 'fuelDeliveryData', 'invoiceItems',
        'invoiceSettings', 'tillPayment', 'fuelPumps_pms', 'fuelPumps_ago', 'fuelExpenses',
        'salesDate', 'shift', 'pmsTankOpening', 'pmsTankClosing', 'agoTankOpening', 'agoTankClosing',
        'pmsPrice', 'agoPrice', 'petrolPrice', 'dieselPrice', 'deliveredTo', 'totalOrder', 'deliveryYear',
        'offloadingRecords', 'tabVisibility', 'tabConfigurations', 'employees', 'payrollRecords',
        'mpesaTransactions', 'reportSettings', 'chatHistory', 'dataBackups'
      ];
      keysToRemove.forEach(key => localStorage.removeItem(`${oldUserKey}${key}`));
    } catch (error) {
  ...
  // Load data from cloud when user changes - but prevent data loss
  useEffect(() => {
    if (user) {
      // First try to load from cloud, fallback to localStorage if cloud fails
      loadFromCloud().catch(() => {
  ...
  // Load data on mount - always load from localStorage first
  useEffect(() => {
    loadFromStorage();
  }, []);
