...
const DEFAULT_TEMPLATES: Template[] = [
  { id: 'welcome', name: 'Welcome Email', subject: 'Welcome to FuelPro', body: 'Hello {{name}},\n\nWelcome to FuelPro! Your account has been created successfully.\n\nStation: {{station}}\nRole: {{role}}\n\nGet started now.', variables: ['name', 'station', 'role'] },
  { id: 'password-reset', name: 'Password Reset', subject: 'Password Reset Request', body: 'Hello {{name}},\n\nA password reset was requested for your account.\n\nUse this code: {{code}}\n\nIf you did not request this, ignore this email.', variables: ['name', 'code'] },
  { id: 'invite', name: 'Invitation', subject: 'Invitation to Join FuelPro', body: 'Hello,\n\nYou have been invited to join {{station}} as {{role}}.\n\nClick here to accept: {{link}}\n\nThis invite expires on {{expires}}.', variables: ['station', 'role', 'link', 'expires'] },
  { id: 'low-inventory', name: 'Low Inventory Alert', subject: 'Low Fuel Inventory Alert', body: 'Hello {{name}},\n\nFuel level for {{fuelType}} is low at {{station}}.\n\nCurrent level: {{level}} liters\nThreshold: {{threshold}} liters\n\nPlease restock soon.', variables: ['name', 'station', 'fuelType', 'level', 'threshold'] },
  { id: 'daily-report', name: 'Daily Report', subject: 'Daily Sales Report', body: 'Hello {{name}},\n\nHere is your daily report for {{station}} on {{date}}:\n\nTotal Sales: {{totalSales}}\nTransactions: {{transactions}}\nVolume: {{volume}}L\n\nHave a great day!', variables: ['name', 'station', 'date', 'totalSales', 'transactions', 'volume'] },
];

function load(): Template[] {
  try { const s = localStorage.getItem(TMPL_KEY); if (s) return JSON.parse(s); } catch { /* */ }
...
export default function EmailTemplatesSection({ logAudit }: Props) {
  ...
  const select = (t: Template) => { setActive({ ...t }); setPreview(false); setSaved(false); };
  ...
  const handleSave = () => {
    ...
    logAudit('Email Template Updated', `Template "${active.name}" modified`, 'success');
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-medium text-white flex items-center gap-2"><Mail size={18} className="text-cyan-400" /> Email Templates</h2>
          <p className="text-xs text-gray-500 mt-0.5">Customize email templates with variables</p>
        </div>
      </div>
        ...
        <div className="col-span-2 bg-[#161618] border border-white/[0.06] rounded-xl p-5">
          {active ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-medium text-white">{active.name}</h3>
                <div className="flex items-center gap-2">
                  {saved && <span className="text-xs text-emerald-400 flex items-center gap-1"><CheckCircle2 size={10} /> Saved</span>}
              ...
              {preview ? (
                <div className="space-y-3">
                  ...
                  <div className="p-3 bg-black/20 rounded-lg whitespace-pre-wrap text-xs text-gray-300 font-mono">{previewBody(active.body)}</div>
                </div>
              ) : (
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-gray-400 mb-1 block">Subject</label>
                    <input value={active.subject}
                  ...
                  <div>
                    ...
                    <textarea value={active.body}
                      onChange={e => setActive({ ...active, body: e.target.value })} rows={10}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {active.variables.map(v => (
                      <span key={v} className="text-[10px] px-1.5 py-0.5 bg-blue-500/10 text-blue-300 rounded border border-blue-500/20">{'{{'}{v}{'}}'}</span>
                    ))}
                  </div>
                </div>
              )}
