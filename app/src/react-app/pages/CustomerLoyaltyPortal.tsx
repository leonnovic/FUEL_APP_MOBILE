...
import { 
  Star, Gift, History, Award, ChevronRight, 
  MapPin, Phone, Mail, TrendingUp, Zap, 
  Settings, LogOut, Shield, Clock
} from 'lucide-react';
...
export default function CustomerLoyaltyPortal({
  ...
  const nextTier = customer.tier === 'Bronze' ? 'Silver' : 
                   ...
                   customer.tier === 'Gold' ? 'Platinum' : null;
  const pointsToNextTier = nextTier && config ? config.tierThresholds[nextTier] - customer.points : 0;
  
  // Filter rewards customer can redeem
  const availableRewards = useMemo(() => 
    rewards.filter(r => {
      if (r.minPointsRequired && customer.points < r.minPointsRequired) return false;
      if (r.remainingQuantity !== undefined && r.remainingQuantity <= 0) return false;
      return true;
    }),
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className={`text-sm ${tierColors.text}`}>{customer.tier} Member</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <p className="text-2xl font-bold text-amber-400">{customer.points.toLocaleString()}</p>
            ...
            <div className="bg-white/10 rounded-xl p-3 text-center">
              <p className="text-2xl font-bold">{(customer.totalSpent / 1000).toFixed(0)}K</p>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-8">
        {activeTab === 'card' && (
          <div className="space-y-4">
            ...
            {/* Station Info */}
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Station Details</h3>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="font-semibold text-gray-900 dark:text-white">
                    {customer.lastVisit === '-' ? 'N/A' : new Date(customer.lastVisit).toLocaleDateString()}
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'rewards' && (
          <div className="space-y-3">
            ...
            {availableRewards.map(reward => {
              ...
              const discount = reward.valueType === 'percentage' 
                ? `${reward.value}% off`
              ...
              return (
                <div 
                  ...
                  className={`bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border-2 ${
                    canRedeem ? 'border-green-200 dark:border-green-800' : 'border-gray-100 dark:border-gray-700'
                  }`}
                >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                      ...
                      tx.type === 'redeem' ? 'bg-amber-100 text-amber-600' :
                      'bg-blue-100 text-blue-600'
                    }`}>
                      {tx.type === 'earn' ? <Zap size={18} /> :
                       tx.type === 'redeem' ? <Gift size={18} /> :
                       <TrendingUp size={18} />}
                    </div>
                    <div>
                  ...
                  <div className={`text-right font-semibold ${
                    ...
                    tx.type === 'redeem' ? 'text-amber-600' :
                    'text-blue-600'
                  }`}>
                    {tx.type === 'earn' ? '+' : tx.type === 'redeem' ? '-' : ''}{Math.abs(tx.points).toLocaleString()} pts
                  </div>
                </div>
              </div>
            ))}
...
export default CustomerLoyaltyPortal;