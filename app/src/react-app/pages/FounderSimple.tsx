...
import {
  ...
  getSWRStats,
  SupabaseStorage,
} from "@/react-app/lib/cloudStorage";
    ...
    try {
      ...
      if (raw) {
        ...
        if (
          s?.active &&
          s?.loginTime &&
          Date.now() - s.loginTime < 8 * 3600 * 1000
        ) {
  ...
  const login = (user: string, pw: string) => {
    if (user === DEFAULT_CREDS.username && pw === DEFAULT_CREDS.password) {
      localStorage.setItem(
        SESSION_KEY,
        JSON.stringify({ username: user, active: true, loginTime: Date.now() })
      );
      setIsAuthenticated(true);
      setUsername(user);
      return true;
...
function setItem(key: string, val: any): void {
  ...
  try {
    ...
    localStorage.setItem(
      "fuelpro_sync_event",
      JSON.stringify({ key, data: val, timestamp: Date.now() })
    );
  }
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      >
        ...
        >
          ...
          >
            <BarChart3 size={14} style={{ color: "#f59e0b" }} /> Revenue Trend
          </h3>
          <div
            style={{ height: 120, display: "flex", alignItems: "end", gap: 4 }}
          >
        ...
        >
          ...
          {logs.length > 0 ? (
            logs.map((item: any, i: number) => (
              <div
                ...
                style={{
                  ...
                  padding: "8px 0",
                  borderBottom:
                    i < logs.length - 1 ? "1px solid #1a1a1a" : "none",
                }}
              >
                <div
                  style={{
            ...
            >
              No recent activity. Use the app to generate activity logs.
            </p>
          )}
        </div>
      </div>
...
function UsersSection() {
  ...
  const filteredUsers =
    filter === "all" ? users : users.filter((u: any) => u.role === filter);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
            Manage platform users and roles (Live Backend)
          </p>
        </div>
      ...
      >
        ...
        >
          ...
          >
            {stats?.byStatus?.find((s: any) => s.status === "active")?.count ||
              0}
          </p>
        </div>
      </div>
          ...
          {filteredUsers && filteredUsers.length > 0 ? (
                ...
                <td style={{ padding: "10px 12px" }}>
                  <select
                    ...
                    onChange={async e => {
                      if (
                        confirm(
                          `Change ${u.name || u.email}'s role to ${e.target.value}?`
                        )
                      ) {
                ...
                <td style={{ padding: "10px 12px" }}>
                  <select
                    ...
                    onChange={async e => {
                      if (
                        confirm(
                          `Change ${u.name || u.email}'s status to ${e.target.value}?`
                        )
                      ) {
            ...
            <tr>
              ...
              >
                {loadingUsers
                  ? "Loading..."
                  : "No users found. Users sign up through the main app."}
              </td>
            </tr>
          )}
        </DataTable>
...
function StationsSection() {
  ...
  const addStation = () => {
    ...
    setStations(updated);
    setItem("fuelpro_stations_v3", updated);
    setNewStation({ name: "", location: "", phone: "" });
    toast("Station created");
  };
  const removeStation = (id: number) => {
    const updated = stations.filter((s: any) => s.id !== id);
    setStations(updated);
    setItem("fuelpro_stations_v3", updated);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      >
        ...
        <KpiCard
          title="Avg Sale"
          value={
            sales.length ? `$${(totalRev / sales.length).toFixed(2)}` : "$0.00"
          }
          icon={Receipt}
          color="#3b82f6"
        />
...
function InventorySection() {
  ...
  const addItem = () => {
    if (!newItem.fuelType.trim()) return;
    const updated = [
      ...items,
      {
        ...newItem,
        ...
        alertThreshold: Number(newItem.alertThreshold),
      },
    ];
    setItems(updated);
    setItem("fuelpro_inventory_v3", updated);
    setNewItem({
      fuelType: "",
      currentStock: "",
      capacity: "",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Fuel stock across all stations
        </p>
      </div>
      <div
        style={{
      ...
      >
        <input
          ...
          onChange={e => setNewItem({ ...newItem, fuelType: e.target.value })}
          style={{
            padding: "8px 12px",
            background: "#1a1a1f",
            border: "1px solid #333",
        ...
        <input
          placeholder="Price/L"
          type="number"
          value={newItem.pricePerLiter}
          onChange={e =>
            setNewItem({ ...newItem, pricePerLiter: e.target.value })
          }
          style={{
            padding: "8px 12px",
            background: "#1a1a1f",
        ...
        <input
          ...
          onChange={e =>
            setNewItem({ ...newItem, alertThreshold: e.target.value })
          }
          style={{
            padding: "8px 12px",
            background: "#1a1a1f",
          ...
          items.map((item: any, i: number) => {
            const fill =
              item.capacity > 0
                ...
                : 0;
            const low =
              item.alertThreshold && item.currentStock < item.alertThreshold;
            return (
              <tr
                key={item.id || i}
              ...
              >
                ...
                <td style={{ padding: "10px 12px" }}>
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 11, color: low ? "#f87171" : "#888" }}
                    >
                      {fill}%
                    </span>
                  </div>
        ...
        {plans.map((plan: any, idx: number) => (
          ...
          >
            ...
            <p style={{ fontSize: 11, color: "#888" }}>
              Users: <span style={{ color: "#fff" }}>{plan.users}</span>{" "}
              &middot; Stations:{" "}
              <span style={{ color: "#fff" }}>{plan.stations}</span>
            </p>
            <ul style={{ margin: "12px 0", padding: 0, listStyle: "none" }}>
              {(plan.features || []).map((f: string, i: number) => (
                ...
                >
                  <CheckCircle2 size={10} style={{ color: "#10b981" }} /> {f}
                </li>
              ))}
            </ul>
            <button
  ...
  const addCoupon = () => {
    ...
    const updated = [
      ...coupons,
      {
        ...
        max: Number(newCoupon.max),
        uses: 0,
        status: "active",
      },
    ];
    setCoupons(updated);
    setItem("fuelpro_coupons", updated);
    setNewCoupon({ code: "", type: "percentage", value: "", max: "" });
    toast("Coupon added");
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {coupons.length > 0 ? (
          coupons.map((c: any, i: number) => (
            <tr key={i} style={{ borderBottom: "1px solid #1a1a1a" }}>
              ...
              <td
                style={{ padding: "10px 12px", color: "#f59e0b", fontSize: 13 }}
              >
...
function SecuritySection() {
  ...
  const [twoFAEnabled, setTwoFAEnabled] = useState(() => {
    try {
      return getItem("fuelpro_founder_2fa", { enabled: false }).enabled;
    } catch {
      return false;
    }
  });
  const changePw = () => {
    ...
    if (currentPw !== DEFAULT_CREDS.password) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          ...
          >
            <Shield size={14} style={{ color: "#f59e0b" }} /> Two-Factor
            Authentication
          </h3>
          ...
          {twoFAEnabled && (
            ...
            >
              <CheckCircle2 size={12} /> 2FA Active
            </p>
          )}
          <button
            onClick={() => {
              ...
              setItem("fuelpro_founder_2fa", { enabled: next });
            ...
            style={{
              ...
              color: twoFAEnabled ? "#f87171" : "#f59e0b",
              border: `1px solid ${twoFAEnabled ? "rgba(239,68,68,0.2)" : "rgba(245,158,11,0.2)"}`,
              borderRadius: 8,
              fontSize: 12,
              cursor: "pointer",
              marginTop: 8,
        ...
        >
          ...
          >
            <Key size={14} style={{ color: "#3b82f6" }} /> Change Password
          </h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {msg && (
              <p
                style={{
                  fontSize: 11,
                  color: msg.includes("updated") ? "#34d399" : "#f87171",
                  margin: 0,
                }}
              >
                {msg}
          ...
          <DataTable headers={["Device", "IP", "Started", "Actions"]}>
            {sessions.map((s: any, i: number) => (
              <tr key={i} style={{ borderBottom: "1px solid #1a1a1a" }}>
                ...
                <td
                  style={{ padding: "8px 12px", color: "#888", fontSize: 11 }}
                >
                  {s.ip || "—"}
                </td>
                <td
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                color: (cfg as any)[key] ? "#10b981" : "#555",
              }}
            >
              {(cfg as any)[key] ? (
...
function ApiKeysSection() {
  ...
  const addKey = () => {
    ...
    setKeys(updated);
    setItem("fuelpro_apikeys", updated);
    setNewKey({ name: "", key: "" });
    toast("API Key added");
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Manage API access tokens and integrations
        </p>
      </div>
      <div
        style={{
...
function BackupsSection() {
  ...
  const createBackup = () => {
    ...
    setBackups(updated);
    setItem("fuelpro_backups", updated);
...
function TabConfigSection() {
  const [tabCfg, setTabCfg] = useState(() =>
  ...
  const save = () => {
    setItem("fuelpro_tabconfig", tabCfg);
    toast("Tab config saved");
  };
  const entries = Object.entries(tabCfg).sort(
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Configure member-view tabs: name, visibility, and order
        </p>
      </div>
            ...
            <td style={{ padding: "10px 12px" }}>
              <button
                onClick={() =>
                  setTabCfg(p => ({
                    ...p,
                    [tabId]: { ...p[tabId], enabled: !p[tabId].enabled },
                  }))
                }
                style={{
                  padding: "2px 8px",
                  ...
                  color: config.enabled ? "#34d399" : "#9ca3af",
                }}
              >
...
function FeatureManagerSection() {
  ...
  const addFeature = () => {
    ...
    const f = {
      ...
      iconIdx: features.length % icons.length,
    };
    const updated = [...features, f];
    setFeatures(updated);
    setItem("fuelpro_custom_features", updated);
    setNewFeature({ label: "", description: "" });
    toast("Feature added");
  };
  ...
  const removeFeature = (id: string) => {
    const updated = features.filter((f: any) => f.id !== id);
    setFeatures(updated);
    setItem("fuelpro_custom_features", updated);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Dynamically add, remove, and toggle platform features without code
          changes
        </p>
      </div>
      <div
        ...
        {features.map((f: any) => {
          ...
          return (
            <tr key={f.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
              <td style={{ padding: "10px 12px" }}>
                <IconComp size={16} style={{ color: "#8b5cf6" }} />
              </td>
              <td
                style={{
                  padding: "10px 12px",
      ...
      {features.length === 0 && (
        ...
        >
          No custom features yet. Add your first feature above.
        </p>
      )}
    </div>
  );
...
function BatchUpdateSection() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          if (parsed.apiKeys) {
            Object.entries(parsed.apiKeys).forEach(([k, v]) =>
              setItem(`fuelpro_apikey_${k}`, v)
            );
            affected.push("API Keys");
          }
          if (parsed.tabConfig) {
            setItem("fuelpro_tabconfig", parsed.tabConfig);
            affected.push("Tab Config");
          }
          if (parsed.config) {
            const cfg = getItem("fuelpro_config", {});
            setItem("fuelpro_config", { ...cfg, ...parsed.config });
            affected.push("Config");
          }
          if (parsed.stations) {
            setItem("fuelpro_stations", parsed.stations);
            affected.push("Stations");
          }
        } catch {}
      }
    }
    const record = {
      id: `batch_${Date.now()}`,
      name: `Batch: ${files.length} file${files.length > 1 ? "s" : ""}`,
      files: uploadedFiles,
      affected: affected.length ? affected : ["Site-wide"],
      timestamp: new Date().toISOString(),
      status: "applied",
    };
    setRecords((p: any) => [record, ...p]);
    setUploading(false);
    setResult(
      `Processed ${files.length} file(s). Affected: ${affected.join(", ") || "Site-wide"}`
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Deploy site-wide changes by uploading JSON config files
        </p>
      </div>
      <div
        style={{
      ...
      >
        ...
        <p style={{ fontSize: 14, color: "#fff", fontWeight: 500, margin: 0 }}>
        ...
        {result && (
          <p style={{ fontSize: 12, color: "#34d399", marginTop: 12 }}>
            {result}
          </p>
        )}
      </div>
        ...
        {history.length > 0 ? (
          history.map((u: any) => (
            ...
            >
              ...
              <td style={{ padding: "10px 12px", color: "#fff", fontSize: 12 }}>
                {u.description}
              </td>
              <td style={{ padding: "10px 12px", color: "#666", fontSize: 11 }}>
              ...
              <td style={{ padding: "10px 12px" }}>
                <span
                  style={{
                    fontSize: 10,
                    color: u.reverted ? "#f87171" : "#34d399",
                  }}
                >
                  {u.reverted ? "Reverted" : "Active"}
                </span>
              </td>
              <td style={{ padding: "10px 12px" }}>
                {!u.reverted && (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
            Monitor all access to Founder Access
          </p>
        </div>
        <button
          onClick={clearLogs}
      ...
      <DataTable headers={["User", "Action", "IP Address", "Time", "Result"]}>
        {filtered.map((l: any, i: number) => (
          <tr key={i} style={{ borderBottom: "1px solid #1a1a1a" }}>
            ...
            <td
              style={{ padding: "10px 12px", color: "#f59e0b", fontSize: 12 }}
            >
              {l.action}
            </td>
            <td
...
function DevConsoleSection() {
  ...
  const [history, setHistory] = useState<
    { cmd: string; result: string; ts: string }[]
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Terminal size={20} style={{ color: "#10b981" }} /> Developer Console
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Execute JavaScript directly against the running application. Use with
          caution.
        </p>
      </div>
      <div
...
function DBManagerSection() {
  ...
  const exportAll = () => {
    ...
    data.forEach(d => {
      all[d.key] = localStorage.getItem(d.key);
    });
    ...
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "fuelpro_db_export.json";
    a.click();
    toast("Database exported");
  };
  const clearAll = () => {
    if (confirm("Delete ALL FuelPro data? This cannot be undone!")) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <div>
          ...
          >
            <HardDrive size={20} style={{ color: "#8b5cf6" }} /> Database
            Manager
          </h2>
          <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
            {data.length} tables &middot; {(totalSize / 1024).toFixed(1)} KB
            total
          </p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Globe size={20} style={{ color: "#06b6d4" }} /> Route Manager
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          All application routes and navigation paths
        </p>
      </div>
              ...
              <span
                style={{
                  ...
                  background: r.auth
                    ? "rgba(59,130,246,0.1)"
                    : "rgba(16,185,129,0.1)",
                  color: r.auth ? "#60a5fa" : "#34d399",
                }}
              >
                {r.auth ? "Required" : "Public"}
              </span>
            </td>
            <td style={{ padding: "10px 12px" }}>
              <button
...
function ThemeSection() {
  ...
  const save = () => {
    setItem("fuelpro_theme", theme);
    toast("Theme saved - reload to apply");
  };
  const colors = [
    "#f59e0b",
    "#3b82f6",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Brush size={20} style={{ color: "#ec4899" }} /> Theme Editor
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Customize the visual appearance of the entire platform
        </p>
      </div>
      <div
        style={{
      ...
      >
        ...
        >
          Preview: This is how your theme looks
        </p>
      </div>
      <button
        onClick={save}
...
function NotificationsSection() {
  ...
  const send = () => {
    ...
    setNotifs(updated);
    setItem("fuelpro_notifications", updated);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Bell size={20} style={{ color: "#f59e0b" }} /> Notification Center
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Info size={20} style={{ color: "#3b82f6" }} /> Environment
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Runtime environment details and browser capabilities
        </p>
      </div>
      <div
        style={{
...
function WebhooksSection() {
  ...
  const addHook = () => {
    ...
    const updated = [...hooks, h];
    setHooks(updated);
    setItem("fuelpro_webhooks", updated);
    setNewHook({ url: "", event: "sale.created", secret: "", active: true });
    toast("Webhook added");
  };
  const toggleHook = (id: number) => {
    const updated = hooks.map((h: any) =>
  ...
  const removeHook = (id: number) => {
    const updated = hooks.filter((h: any) => h.id !== id);
    setHooks(updated);
    setItem("fuelpro_webhooks", updated);
  };
  const testHook = (url: string) => {
    toast("Test request sent to " + url.substring(0, 30) + "...");
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Wifi size={20} style={{ color: "#06b6d4" }} /> Webhook Manager
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Configure HTTP webhooks for real-time event notifications
        </p>
      </div>
      <div
        style={{
      ...
      >
        ...
        <select
          value={newHook.event}
          onChange={e => setNewHook({ ...newHook, event: e.target.value })}
          style={{
            padding: "8px 12px",
            background: "#1a1a1f",
            border: "1px solid #333",
      ...
      <DataTable headers={["URL", "Event", "Secret", "Status", "Actions"]}>
        {hooks.map((h: any) => (
          <tr key={h.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
            ...
            <td
              style={{ padding: "10px 12px", color: "#f59e0b", fontSize: 11 }}
            >
              {h.event}
            </td>
            <td style={{ padding: "10px 12px", color: "#666", fontSize: 11 }}>
...
function CacheSection() {
  const [swStatus, setSwStatus] = useState<"unknown" | "active" | "none">(
    "unknown"
  );
  useEffect(() => {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker
        .getRegistration()
        .then(r => setSwStatus(r ? "active" : "none"));
    } else {
      setSwStatus("none");
    }
  }, []);
  ...
  const clearLS = () => {
    if (confirm("Clear ALL FuelPro localStorage data?")) {
      for (let i = localStorage.length - 1; i >= 0; i--) {
        const k = localStorage.key(i);
        if (k?.startsWith("fuelpro_")) localStorage.removeItem(k);
      }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Server size={20} style={{ color: "#ef4444" }} /> Cache Control
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
      ...
      >
        ...
        >
          <Cpu
            size={24}
            style={{
              color: swStatus === "active" ? "#10b981" : "#666",
              margin: "0 auto 8px",
            }}
          />
          <p
            style={{ fontSize: 13, color: "#fff", margin: 0, fontWeight: 500 }}
          >
            Service Worker
          </p>
          <p
            style={{
              fontSize: 11,
              color: swStatus === "active" ? "#34d399" : "#888",
              margin: "4px 0 12px",
            }}
          >
            {swStatus === "active"
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              fontSize: 11,
              cursor: swStatus === "active" ? "pointer" : "not-allowed",
              opacity: swStatus === "active" ? 1 : 0.4,
            }}
          >
            Unregister
          </button>
...
function CloudConfigSection() {
  const [provider, setProvider] = useState(() => {
    ...
    if (config) {
      try {
        return JSON.parse(config).provider || "supabase";
      } catch {}
    }
    return "supabase";
  });
  const [endpoint, setEndpoint] = useState(() => {
    ...
    if (config) {
      try {
        return JSON.parse(config).apiEndpoint || "";
      } catch {}
    }
    return "";
  });
  ...
  const [seafileUsername, setSeafileUsername] = useState(() => {
    ...
    if (config) {
      try {
        return JSON.parse(config).seafileUsername || "";
      } catch {}
    }
    return "";
  });
  const [seafilePassword, setSeafilePassword] = useState(() => {
    ...
    if (config) {
      try {
        return JSON.parse(config).seafilePassword || "";
      } catch {}
    }
    return "";
  });
  const [seafileLibId, setSeafileLibId] = useState(() => {
    ...
    if (config) {
      try {
        return JSON.parse(config).seafileLibId || "";
      } catch {}
    }
    return "";
  });
  const [seafileLibs, setSeafileLibs] = useState<
  ...
  const loadSeafileLibs = async () => {
    if (
      provider !== "seafile" ||
      !endpoint ||
      (!apiKey && (!seafileUsername || !seafilePassword))
    )
      return;
    ...
    try {
      ...
      console.error("Failed to load Seafile libs", e);
    }
  ...
  const handleSave = () => {
    ...
    if (sync) {
      ...
      toast(
        `${provider.charAt(0).toUpperCase() + provider.slice(1)} sync configured!`
      );
    }
  };
  ...
  const providers = [
    {
      ...
      label: "Supabase",
      icon: "🔺",
      desc: "PostgreSQL + Realtime + Storage",
    },
    {
      id: "firebase",
      label: "Firebase",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Enterprise-grade multi-cloud data synchronization
        </p>
      </div>
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          >
            <Upload
              size={20}
              style={{ color: status.pendingChanges > 0 ? "#f59e0b" : "#666" }}
            />
          </div>
          ...
          <p
            style={{
              fontSize: 14,
              fontWeight: "bold",
              color: status.pendingChanges > 0 ? "#f59e0b" : "#888",
              margin: "4px 0 0",
            }}
          >
          ...
          {providers.map(p => (
            <button
              ...
              style={{
                padding: "12px 16px",
                background:
                  provider === p.id ? "rgba(245,158,11,0.15)" : "transparent",
      ...
      >
        <p style={{ fontSize: 12, color: "#60a5fa", margin: 0 }}>
          <strong>Enterprise Features:</strong> Multi-provider sync (Supabase,
          Firebase, Seafile, Custom API) • SWR caching for instant loads •
          Real-time WebSocket updates • Offline-first architecture •
          Cross-device session management via Upstash Redis • Zero-egress file
...
function AIBatchSection() {
  ...
  const [history, setHistory] = useState<
    Array<{ prompt: string; result: string; time: string }>
  >(() => {
    try {
      const h = localStorage.getItem("fuelpro_founder_ai_history");
  ...
  const processPrompt = () => {
    ...
    setTimeout(() => {
      ...
      if (
        p.includes("price") &&
        (p.includes("petrol") || p.includes("diesel") || p.includes("fuel"))
      ) {
        const prices = p.match(/(\d+(?:\.\d+)?)/g);
        if (prices && prices.length >= 1) {
          const fuelPrices = JSON.parse(
          ...
          if (p.includes("petrol"))
            fuelPrices[stationId].petrol = parseFloat(prices[0]);
          if (p.includes("diesel") && prices.length > 1)
            fuelPrices[stationId].diesel = parseFloat(prices[1]);
          if (p.includes("diesel") && prices.length === 1)
            fuelPrices[stationId].diesel = parseFloat(prices[0]);

          localStorage.setItem(
            "fuelpro_fuel_prices_v2",
            JSON.stringify(fuelPrices)
          );
...
Synced to Price Board.`;
      ...
      // Add expense
        ...
        const name = nameMatch
          ...
          : prompt.split(" ").slice(2, 4).join(" ");
        const amountMatch =
          p.match(/amount\s+(\d+(?:\.\d+)?)/) || p.match(/(\d+(?:\.\d+)?)/);
...
• Updated: ${new Date().toLocaleString()}`;
      ...
      // Inventory alert
        ...
        const fuelType = p.includes("diesel")
          ? "diesel"
          : p.includes("petrol")
            ? "petrol"
            : "all";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Describe what you want to do in plain English. The AI will process
          batch operations across all data.
        </p>
      </div>
      ...
      >
        ...
        >
          <button
            ...
            style={{
              padding: "8px 20px",
              background: isProcessing ? "#444" : "#f59e0b",
      ...
      <div>
        <p style={{ fontSize: 11, color: "#888", marginBottom: 8 }}>
          <Lightbulb size={11} style={{ display: "inline" }} /> Examples — click
          to use:
        </p>
...
function TrialManagerSection() {
  ...
  const resetTrial = () => {
    localStorage.removeItem("fuelpro_trial");
    setTrials([]);
    toast("Trial reset");
  };
  const grantPaid = () => {
    ...
    setTrials(t =>
      t.map(x => ({ ...x, status: "paid", minutesLeft: Infinity }))
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Clock size={20} style={{ color: "#f59e0b" }} /> Trial Manager
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Manage user trial periods and subscription status
        </p>
      </div>
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            {trials.filter(t => t.status === "paid").length}
          </p>
        </div>
        <div
          style={{
        ...
        >
          ...
          >
            {
              trials.filter(t => t.status === "active" && t.minutesLeft <= 0)
                .length
            }
          </p>
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        >
          <Building2 size={20} style={{ color: "#3b82f6" }} /> Company Profile
        </h2>
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          View and manage registered company profiles
        </p>
      </div>
      {profile ? (
        <div
        ...
        >
          ...
          <p style={{ fontSize: 12, color: "#666", margin: "4px 0 0" }}>
            Users can add company info during signup.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "#666", margin: "4px 0 0" }}>
          Manage paywall tiers and subscriber data
        </p>
      </div>
      ...
      >
        ...
        <KpiCard
          title="Staff (Ksh 299)"
          value={tierCounts["staff"] || 0}
          icon={User}
          color="#3b82f6"
        />
        <KpiCard
          title="Manager (Ksh 999)"
          value={tierCounts["manager"] || 0}
          icon={Crown}
          color="#f59e0b"
        />
        <KpiCard
          title="Auditor (Ksh 2499)"
          value={tierCounts["auditor"] || 0}
          icon={Shield}
          color="#10b981"
        />
      </div>
          ...
          >
            ...
            >
              <span style={{ fontSize: 11, color: "#666" }}>
                {tierCounts[t.key] || 0} subscribers
              </span>
              <span
                style={{
                  padding: "2px 8px",
                ...
                <td style={{ padding: "10px 12px" }}>
                  <span
                    style={{
                      ...
                      borderRadius: 10,
                      fontSize: 10,
                      background: `${tierColors[s.tier] || "#94a3b8"}15`,
                      color: tierColors[s.tier] || "#94a3b8",
                      fontWeight: 600,
                      textTransform: "capitalize",
                    }}
                  >
...
function renderSection(id: SectionId) {
  switch (id) {
    ...
    case "tabconfig":
      return <TabConfigSection />;
    case "featuremgr":
      return <FeatureManagerSection />;
    case "batchupdate":
      return <BatchUpdateSection />;
    case "updatehist":
      return <UpdateHistorySection />;
    case "accesslogs":
      return <AccessLogsSection />;
    case "devconsole":
      return <DevConsoleSection />;
    case "dbmanager":
      return <DBManagerSection />;
    case "routes":
    ...
    case "cache":
      return <CacheSection />;
    case "trialmgr":
      return <TrialManagerSection />;
    case "companyprofile":
      return <CompanyProfileSection />;
    case "subscriptions":
      return <SubscriptionsSection />;
    case "aibatch":
      return <AIBatchSection />;
...
function FounderLogin({
  ...
  onLogin: (user: string, pw: string) => boolean;
}) {
      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        ...
        >
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 12, color: "#666", margin: "4px 0 0" }}>
              FuelPro Backend Administration
            </p>
          </div>
          {err && (
            <div
...
export default function FounderAccess() {
  const { isAuthenticated, isLoading, username, logout, login } =
    useFounderAuth();
  ...
  const integrationsSections = SECTIONS.filter(
