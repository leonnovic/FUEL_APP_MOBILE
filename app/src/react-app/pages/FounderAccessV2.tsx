      ...
      <div className="w-full max-w-md">
        <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            >
              <Shield size={16} />{" "}
              {isSubmitting ? "Authenticating..." : "Access Backend"}
            </button>
          </div>
        </div>
      </div>
...
function DashboardSection() {
  const [sales] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("fuelpro_sales") || "[]");
    } catch {
      return [];
    }
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Overview of your FuelPro platform
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4">
          <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
            <BarChart3 size={14} className="text-amber-400" /> Revenue Trend
          </h3>
            ...
            {[
              {
                ...
                detail: "Fuel sale #1042",
                time: "2 min ago",
                color: "text-emerald-400",
              },
              {
                action: "Inventory updated",
                detail: "Tank 1 refilled",
                time: "15 min ago",
                color: "text-blue-400",
              },
              {
              ...
              {
                action: "Price change",
                detail: "Petrol updated",
                time: "3 hours ago",
                color: "text-amber-400",
              },
              {
                action: "Backup created",
                detail: "Daily auto-backup",
                time: "5 hours ago",
                color: "text-gray-400",
              },
            ].map((item, i) => (
...
function UsersSection() {
  const [users, setUsers] = useState<any[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("fuelpro_users") || "[]");
    } catch {
      return [];
    }
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="px-3 py-2 bg-amber-500/15 text-amber-400 rounded-lg text-xs border border-amber-500/20 flex items-center gap-1">
...
function StationsSection() {
  const [stations] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("fuelpro_stations") || "[]");
    } catch {
      return [];
    }
  });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage all fuel stations</p>
        </div>
        <button className="px-3 py-2 bg-amber-500/15 text-amber-400 rounded-lg text-xs border border-amber-500/20 flex items-center gap-1">
      ...
      <DataTable
        headers={["ID", "Name", "Location", "Status", "Revenue", "Actions"]}
      >
        {stations.length > 0 ? (
          stations.map((s: any, i: number) => (
            ...
            >
              ...
              <td className="px-4 py-3">
                <span
                  className={`px-2 py-0.5 rounded-full text-[10px] ${s.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"}`}
                >
                  {s.status || "active"}
                </span>
              </td>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">All sales transactions</p>
      </div>
        ...
        {sales.length > 0 ? (
          sales.map((s: any, i: number) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-white capitalize text-sm">
                {s.fuelType || "—"}
              </td>
          ...
          <tr>
            <td
              ...
              className="px-4 py-8 text-center text-gray-500 text-sm"
            >
              No sales recorded yet. Start adding sales from the main app.
            </td>
          </tr>
        )}
      </DataTable>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <KpiCard
          ...
          value={
            items.filter((i: any) => i.currentStock < i.alertThreshold).length
          }
          icon={AlertTriangle}
          color="text-red-400"
        />
        <KpiCard
          ...
          value={`${items.reduce((sum: number, i: any) => sum + (Number(i.capacity) || 0), 0).toLocaleString()}L`}
          icon={Fuel}
          color="text-emerald-400"
        />
      </div>
          ...
          items.map((item: any, i: number) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${low ? "bg-red-500" : fill > 80 ? "bg-emerald-500" : "bg-amber-500"}`}
                        style={{ width: `${Math.min(fill, 100)}%` }}
                      />
                    </div>
                    <span
                      className={`text-xs ${low ? "text-red-400" : "text-gray-400"}`}
                    >
                      {fill}%
                    </span>
                  </div>
                ...
                <td className="px-4 py-3">
                  {low ? (
                    <span className="px-2 py-0.5 rounded-full text-[10px] bg-red-500/10 text-red-400">
                      Low
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 rounded-full text-[10px] bg-emerald-500/10 text-emerald-400">
          ...
          >
            ...
            <div className="mt-4 space-y-2">
              <p className="text-xs text-gray-400">
                Users: <span className="text-white">{plan.users}</span>
              </p>
              <p className="text-xs text-gray-400">
                Stations: <span className="text-white">{plan.stations}</span>
              </p>
            </div>
            ...
            <div className="mt-4 flex gap-2">
              <button className="flex-1 py-2 bg-amber-500/15 text-amber-400 rounded-lg text-xs border border-amber-500/20 hover:bg-amber-500/25 transition-colors">
                Edit
              </button>
              <button className="py-2 px-3 bg-gray-800 text-gray-400 rounded-lg text-xs hover:text-white transition-colors">
                <Settings size={12} />
              </button>
            </div>
          </div>
      ...
      >
        {coupons.map((c, i) => (
          ...
          >
            ...
            <td className="px-4 py-3">
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] ${c.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"}`}
              >
                {c.status}
              </span>
            </td>
...
function SecuritySection() {
  ...
  const handleChangePassword = () => {
    ...
    const storedCreds = JSON.parse(
      localStorage.getItem("fuelpro_founder_creds") || "{}"
    );
    if (currentPw !== (storedCreds.password || DEFAULT_CREDS.password)) {
      setPwError("Current password incorrect");
      return;
    }
    // Save new password
    localStorage.setItem(
      "fuelpro_founder_creds",
      JSON.stringify({ ...storedCreds, password: newPw })
    );
  ...
  const handleEnable2FA = async () => {
    if (twoFAStep === "qr") {
      ...
      if (verifyCode(twoFASecret, twoFACode)) {
        localStorage.setItem(
          "fuelpro_founder_2fa",
          JSON.stringify({ enabled: true, secret: btoa(twoFASecret) })
        );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-5">
          <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
            <Shield size={14} className="text-amber-400" /> Two-Factor
            Authentication
          </h3>
          <p className="text-xs text-gray-500 mb-3">
            {twoFAEnabled ? "2FA is active." : "Protect with TOTP-based 2FA."}
          </p>
          {twoFAEnabled && (
            <div className="flex items-center gap-2 mb-3 text-emerald-400 text-xs">
          ...
          <button
            ...
            className="px-4 py-2 bg-amber-500/15 hover:bg-amber-500/25 text-amber-400 rounded-lg text-xs border border-amber-500/20 transition-colors"
          >
            {twoFAEnabled ? "Manage 2FA" : "Configure 2FA"}
          </button>
        </div>
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-5">
          <h3 className="text-sm font-medium text-white mb-3 flex items-center gap-2">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full py-2 bg-blue-500/15 hover:bg-blue-500/25 text-blue-400 rounded-lg text-xs border border-blue-500/20"
            >
              Update Password
            </button>
          </div>
            ...
            {twoFAStep === "qr" ? (
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-amber-500 disabled:bg-gray-700 text-white rounded-xl text-sm font-medium"
                >
                  Verify
                </button>
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-4">
        ...
        <div className="h-48 flex items-end gap-1">
          {Array.from({ length: 30 }, (_, i) => 20 + Math.random() * 60).map(
            (h, i) => (
              <div
                key={i}
...
function ConfigSection() {
  ...
  return (
    <div className="space-y-4">
      ...
      <button className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-medium flex items-center gap-2">
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage API access tokens</p>
        </div>
        <button className="px-3 py-2 bg-amber-500/15 text-amber-400 rounded-lg text-xs border border-amber-500/20 flex items-center gap-1">
      ...
      {keys.map((k, i) => (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span
              className={`px-2 py-0.5 rounded-full text-[10px] ${k.status === "active" ? "bg-emerald-500/10 text-emerald-400" : "bg-gray-500/10 text-gray-400"}`}
            >
              {k.status}
            </span>
      ...
      <DataTable headers={["Name", "Date", "Size", "Status", "Actions"]}>
        {backups.map((b, i) => (
          ...
          >
            ...
            <td className="px-4 py-3">
              <span
                className={`px-2 py-0.5 rounded-full text-[10px] ${b.status === "success" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
              >
                {b.status}
              </span>
            </td>
...
export default function FounderAccessV2() {
  ...
  if (!isAuthenticated) {
    return (
      <FounderLogin
        onLogin={() => {
          /* state update in login() already triggers re-render */
        }}
      />
    );
  }
  ...
  const sidebar = (
    <aside
      className={`${sidebarCollapsed ? "w-16" : "w-64"} bg-gray-900 border-r border-gray-800 flex flex-col transition-all duration-300`}
    >
      <div className="p-4 flex items-center justify-between border-b border-gray-800">
        ...
        <button
          ...
          className="text-gray-500 hover:text-white hidden md:block"
        >
          {sidebarCollapsed ? (
