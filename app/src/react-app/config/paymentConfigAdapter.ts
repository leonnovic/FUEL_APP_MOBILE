import { WORLD_PAYMENT_CONFIGS, CountryPaymentConfig } from './worldPaymentConfigs';
...
function getFlagEmoji(countryCode: string): string {
  ...
  return String.fromCodePoint(FLAG_OFFSET + (cp1 - A_CODE), FLAG_OFFSET + (cp2 - A_CODE));
}
...
export const SORTED_COUNTRY_CODES = Object.keys(COUNTRY_CONFIGS).sort(
  (a, b) => COUNTRY_CONFIGS[a].name.localeCompare(COUNTRY_CONFIGS[b].name)
);

// ─── Get country config safely ───
export function getCountryConfig(code: string): LegacyCountryConfig | undefined {
