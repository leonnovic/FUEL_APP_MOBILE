...
export class AuditLogger {
  ...
  private getDefaultSeverity(action: AuditAction): AuditSeverity {
    const criticalActions: AuditAction[] = [
      'USER_DELETE', 'STATION_DELETE', 'DATA_DELETE', 'SYSTEM_CONFIG'
    ];
    const warningActions: AuditAction[] = [
      'LOGIN_FAILED', 'USER_ROLE_CHANGE', 'API_KEY_REVOKE', 'SETTINGS_UPDATE'
    ];
  ...
  exportLogs(format: 'json' | 'csv' = 'json'): string {
    ...
    // CSV format
    const headers = [
      'ID', 'Timestamp', 'Action', 'Severity', 'User', 'Role',
      'Target Type', 'Target ID', 'Description', 'Success', 'IP Address'
    ];
...
export const auditLog = {
  ...
  userCreate: (user: AdminUser, newUser: { id: string; email: string; name: string }) => {
    AuditLogger.getInstance().log({
      action: 'USER_CREATE',
      user,
      targetType: 'user',
  ...
  userUpdate: (user: AdminUser, targetUser: { id: string; email: string }, changes: Record<string, { old: any; new: any }>) => {
    AuditLogger.getInstance().log({
      action: 'USER_UPDATE',
      user,
      targetType: 'user',
  ...
  roleChange: (user: AdminUser, targetUser: { id: string; email: string }, oldRole: string, newRole: string) => {
    AuditLogger.getInstance().log({
  ...
  settingsUpdate: (user: AdminUser, changes: Record<string, { old: any; new: any }>) => {
    AuditLogger.getInstance().log({
  ...
  stationUpdate: (user: AdminUser, station: { id: string; name: string }, changes: Record<string, { old: any; new: any }>) => {
    AuditLogger.getInstance().log({
