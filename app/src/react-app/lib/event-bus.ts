...
class EventBus {
  ...
  private emitInternal(type: string, payload: any) {
    ...
    if (handlers) {
      handlers.forEach(h => { try { h(payload); } catch (e) { console.error(`Event handler error for ${type}:`, e); } });
    }
  }
  ...
  on(type: string, handler: EventHandler): () => void {
    ...
    return () => { this.handlers[type] = this.handlers[type].filter(h => h !== handler); };
  }
  ...
  emit(type: string, payload?: any) {
    ...
    if (this.channel) {
      try { this.channel.postMessage({ type, payload }); } catch { /* */ }
    }
    // Also dispatch DOM event for within-tab listeners
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent(`fuelpro:${type}`, { detail: payload }));
    }
  }
...
export function onEvent(type: string, handler: EventHandler) { return bus.on(type, handler); }
export function emitEvent(type: string, payload?: any) { bus.emit(type, payload); }
