...
import { usePlatformData, PlatformStats } from '@/react-app/context/PlatformDataContext';
import { 
  TrendingUp, TrendingDown, DollarSign, ShoppingCart, Users, 
  Building2, Package, Activity, Clock, RefreshCw, BarChart3,
  Fuel, CreditCard, AlertTriangle, CheckCircle2
} from 'lucide-react';
...
function KpiCard({ title, value, icon: Icon, color, trend, change, subtitle }: KpiCardProps) {
  return (
    <div style={{
      background: '#111',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 11, color: '#666', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          {title}
        </span>
...
function RevenueChart({ sales }: { sales: Array<{ date: string; total: number }> }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h3 style={{ fontSize: 13, fontWeight: 500, color: '#fff', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          <BarChart3 size={14} style={{ color: '#f59e0b' }} /> Revenue (Last 7 Days)
        </h3>
      </div>
...
function FuelTypeChart({ sales }: { sales: Array<{ fuelType: string; total: number }> }) {
  const fuelTypes = sales.reduce((acc, s) => {
    const type = s.fuelType || 'Unknown';
    acc[type] = (acc[type] || 0) + (s.total || 0);
    return acc;
  }, {} as Record<string, number>);

  const total = Object.values(fuelTypes).reduce((sum, val) => sum + val, 0) || 1;
  ...
  return (
    <div style={{
      ...
      <h3 style={{ fontSize: 13, fontWeight: 500, color: '#fff', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {Object.keys(fuelTypes).length > 0 ? (
        ...
        <p style={{ textAlign: 'center', color: '#555', fontSize: 12 }}>No sales data yet</p>
      )}
    </div>
  );
}
...
function RecentActivity({ getRecentActivity }: { 
  getRecentActivity: (limit?: number) => Array<{ id: string; action: string; details: string; time: string; color: string }> 
}) {
  ...
  return (
    <div style={{
      ...
      {activities.length > 0 ? (
          ...
          {activities.map((item, i) => (
            <div key={item.id || i} style={{ 
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 12, color: '#fff', margin: 0 }}>{item.action}</p>
                <p style={{ fontSize: 10, color: '#555', margin: 0 }}>{item.details}</p>
              </div>
        ...
        <p style={{ textAlign: 'center', color: '#555', fontSize: 12 }}>No activity yet. Use the app to generate activity.</p>
      )}
    </div>
  );
}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {checks.map((check, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {check.status === 'healthy' && <CheckCircle2 size={12} style={{ color: '#10b981' }} />}
            {check.status === 'warning' && <AlertTriangle size={12} style={{ color: '#f59e0b' }} />}
            {check.status === 'error' && <AlertTriangle size={12} style={{ color: '#ef4444' }} />}
            <div style={{ flex: 1 }}>
...
export default function PlatformAnalytics() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 'bold', color: '#fff', margin: 0 }}>
            Platform Analytics
          </h2>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 16 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{sales.length}</div>
            <div style={{ fontSize: 11, color: '#666' }}>Total Sales Records</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{users.length}</div>
            <div style={{ fontSize: 11, color: '#666' }}>Total Users</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{stations.length}</div>
            <div style={{ fontSize: 11, color: '#666' }}>Total Stations</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 24, fontWeight: 'bold', color: '#fff' }}>{inventory.length}</div>
