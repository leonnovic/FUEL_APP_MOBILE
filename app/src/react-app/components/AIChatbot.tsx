...
import { Send, Mic, MicOff, X, Volume2, VolumeX, Bot, User, Minimize2, Maximize2, Sparkles, RefreshCw, AlertCircle } from 'lucide-react';
...
export default function AIChatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      type: 'assistant',
      content: 'Hello! I\'m your FuelPro AI Assistant powered by Google Gemini. I have access to all your business data - sales, deliveries, invoices, M-PESA transactions, payroll, and more. Ask me anything about your fuel station operations!',
      timestamp: new Date()
    }
  ]);
  ...
  const buildBusinessContext = () => {
    ...
    context.tankLevels = {
      pms: { opening: state.pmsTankOpening, closing: state.pmsTankClosing, consumed: state.pmsTankOpening - state.pmsTankClosing },
      ago: { opening: state.agoTankOpening, closing: state.agoTankClosing, consumed: state.agoTankOpening - state.agoTankClosing }
    };

    // TODAY'S SALES - Full pump details
    const pmsTotal = state.pmsPumps.reduce((sum, p) => sum + (Number(p.salesKsh) || 0), 0);
    const agoTotal = state.agoPumps.reduce((sum, p) => sum + (Number(p.salesKsh) || 0), 0);
    const pmsLitres = state.pmsPumps.reduce((sum, p) => sum + (Number(p.salesL) || 0), 0);
    const agoLitres = state.agoPumps.reduce((sum, p) => sum + (Number(p.salesL) || 0), 0);
    const totalExpenses = state.expenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0);
    
    context.todaySales = {
      date: state.salesDate,
      shift: state.shift,
      petrol: { litres: pmsLitres, amount: pmsTotal, pumpCount: state.pmsPumps.length, pumps: state.pmsPumps },
      diesel: { litres: agoLitres, amount: agoTotal, pumpCount: state.agoPumps.length, pumps: state.agoPumps },
      ...
      totalExpenses,
      netIncome: state.tillPayment + ((pmsTotal + agoTotal) - totalExpenses - state.tillPayment)
    };

    // DELIVERY TRACKER - All delivery data
    if (state.deliveryData.rows.length > 0) {
      const deliveries = state.deliveryData.rows;
      const totalSupplied = deliveries.reduce((sum, d) => sum + (Number(d.amount) || 0), 0);
      const totalDebt = deliveries.reduce((sum, d) => sum + (Number(d.debt) || 0), 0);
      const uniqueCustomers = [...new Set(deliveries.map(d => d.name))].filter(n => n);
    ...
    if (Object.keys(state.clients).length > 0) {
      const clientsList = Object.entries(state.clients).map(([name, data]: [string, any]) => ({
        name,
        phone: data.phone,
      ...
      context.clients = {
        ...
        topDebtors: clientsWithDebt.sort((a, b) => b.balance - a.balance).slice(0, 10),
    ...
    context.invoices = {
      ...
      currentTotal: state.invoiceItems.reduce((sum, item) => sum + (Number(item.total) || 0), 0),
    ...
    if (state.offloadingRecords.length > 0) {
      const totalOffloaded = state.offloadingRecords.reduce((sum, r) => sum + (Number(r.quantity) || 0), 0);
      const totalCost = state.offloadingRecords.reduce((sum, r) => sum + (Number(r.totalAmount) || 0), 0);
      const suppliers = [...new Set(state.offloadingRecords.map(r => r.supplier))].filter(s => s);
    ...
    if (state.mpesaTransactions.length > 0) {
      const totalMpesa = state.mpesaTransactions.reduce((sum, t) => sum + (Number(t.amount) || 0), 0);
    ...
    if (state.employees.length > 0) {
      const totalPayroll = state.employees.reduce((sum, e) => sum + (Number(e.basicSalary) || 0), 0);
      ...
      context.payroll = {
        ...
        positions: [...new Set(state.employees.map(e => e.position))].filter(r => r),
    ...
    if (Object.keys(state.salesHistory).length > 0) {
      ...
      context.salesHistory = {
        ...
        dateRange: { from: salesDates[0], to: salesDates[salesDates.length - 1] },
  ...
  const generateLocalResponse = (message: string, context: any): string => {
    ...
    const { todaySales, deliveryTracker, invoices, fuelPrices, tankLevels, payroll, offloading, salesHistory } = context;

    // Today's Sales queries
    if (lowerMsg.includes('today') && (lowerMsg.includes('sale') || lowerMsg.includes('revenue') || lowerMsg.includes('income'))) {
    ...
    // Debt queries
    if (lowerMsg.includes('debt') || lowerMsg.includes('outstanding') || lowerMsg.includes('owe')) {
    ...
    // Payroll queries
    if (lowerMsg.includes('payroll') || lowerMsg.includes('staff') || lowerMsg.includes('employee') || lowerMsg.includes('salary')) {
    ...
    // Offloading queries
    if (lowerMsg.includes('offload') || lowerMsg.includes('supply') || lowerMsg.includes('delivery from')) {
    ...
    // M-PESA queries
    if (lowerMsg.includes('mpesa') || lowerMsg.includes('mobile') || lowerMsg.includes('payment')) {
    ...
    // Business overview
    if (lowerMsg.includes('overview') || lowerMsg.includes('summary') || lowerMsg.includes('status') || lowerMsg.includes('business')) {
  ...
  const sendMessage = async (message: string, isRetry = false) => {
    ...
    try {
      ...
      const assistantMessage: Message = {
        ...
        content: response,
        timestamp: new Date(),
        suggestions: ['Today\'s Sales', 'Outstanding Debts', 'Business Overview', 'Fuel Prices']
      };
      ...
      const errorMessage: Message = {
        ...
        isError: true,
        canRetry: true,
        suggestions: ['What are today\'s sales?', 'Show outstanding debts', 'Business overview']
      };
    ...
    setMessages([
      {
        id: '1',
        type: 'assistant',
        content: 'Chat cleared! I still have access to all your business data. What would you like to know?',
        timestamp: new Date()
      }
    ]);
  };
  ...
  const quickActions = [
    { label: "Today's Sales", query: "Give me a summary of today's sales including total litres and amount for petrol and diesel" },
    { label: "Outstanding Debts", query: "Show me the top customers with outstanding debts and the total debt amount" },
    { label: "Business Overview", query: "Give me a complete overview of my fuel station business including sales, debts, and recent transactions" },
    { label: "M-PESA Summary", query: "Summarize my recent M-PESA transactions" }
  ];
  ...
  return (
    <div className={`fixed bottom-20 md:bottom-6 right-2 md:right-6 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-600 z-50 transition-all duration-300 ${
      isMinimized ? 'w-80 h-16' : 'w-[calc(100vw-16px)] md:w-[420px] h-[70vh] md:h-[650px] max-h-[650px]'
    }`}>
      {/* Header */}
