...
import { Bell, X, AlertTriangle, Info, CheckCircle2, Fuel, FileText, Shield, Calendar } from 'lucide-react';
...
export default function RegulatoryAlerts({ countryCode }: RegulatoryAlertsProps) {
  const { regulatoryUpdates, markUpdateRead, dismissUpdate, unreadCount, highPriorityCount } = useAutoSync(countryCode);
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bell size={14} className={highPriorityCount > 0 ? 'text-red-500' : unreadCount > 0 ? 'text-amber-500' : 'text-gray-400'} />
          <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">
            Regulatory Alerts
          </span>
          {unreadCount > 0 && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${highPriorityCount > 0 ? 'bg-red-500 text-white' : 'bg-amber-500 text-white'}`}>
              {unreadCount}
            </span>
          )}
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2 flex-1 min-w-0">
                  <Icon size={14} className={`flex-shrink-0 mt-0.5 ${update.priority === 'high' ? 'text-red-400' : update.priority === 'medium' ? 'text-amber-400' : 'text-blue-400'}`} />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h5 className="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate">
                        {update.title}
                      </h5>
                      <span className={`text-[9px] px-1 rounded ${update.priority === 'high' ? 'bg-red-500/20 text-red-400' : update.priority === 'medium' ? 'bg-amber-500/20 text-amber-400' : 'bg-blue-500/20 text-blue-400'}`}>
