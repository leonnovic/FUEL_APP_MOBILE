...
import { exportDebtPDF, exportDebtExcel, exportDebtTXT } from '@/react-app/utils/exportUtils';
import { formatAmountWithCommas, parseNumberFromFormatted, formatNumber } from '@/react-app/utils/formatUtils';
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="card">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-2xl font-bold text-blue-900 dark:text-blue-200">Fuel Debt Payment Reminder</h2>
      ...
      <div className="card">
        ...
        <div className="history-panel">
          {Object.keys(state.debtHistory)
            .sort((a, b) => new Date(b.split('_')[0]).getTime() - new Date(a.split('_')[0]).getTime())
            .map((key) => {
              const item = state.debtHistory[key];
              return (
                <div key={key} className="history-item">
                  <span>{item.name} - Ksh {item.amount}</span>
                  <div className="flex gap-2">
                    <button onClick={() => loadDebt(key)} className="text-xs">Load</button>
                    <button onClick={() => deleteDebt(key)} className="text-xs">Delete</button>
                  </div>
                </div>
              );
            })}
