...
import { Plus, Save, Trash2, Truck, Fuel, FileText, Calendar, X } from 'lucide-react';
...
export default function FuelOffloading() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<OffloadingRecord | null>(null);
  ...
  const handleInputChange = (field: keyof OffloadingRecord, value: any) => {
    ...
    if (field === 'quantity' || field === 'rate') {
      const quantity = field === 'quantity' ? parseFloat(value) || 0 : formData.quantity || 0;
  ...
  const saveRecord = () => {
    if (!formData.truckReg || !formData.driverName || !formData.supplier || !formData.quantity || !formData.rate) {
  ...
  // Calculate totals
  const totals = {
    totalQuantity: state.offloadingRecords.reduce((sum, record) => sum + record.quantity, 0),
    totalAmount: state.offloadingRecords.reduce((sum, record) => sum + record.totalAmount, 0),
    pmsQuantity: state.offloadingRecords.filter(r => r.fuelType === 'PMS').reduce((sum, record) => sum + record.quantity, 0),
    agoQuantity: state.offloadingRecords.filter(r => r.fuelType === 'AGO').reduce((sum, record) => sum + record.quantity, 0),
    pmsAmount: state.offloadingRecords.filter(r => r.fuelType === 'PMS').reduce((sum, record) => sum + record.totalAmount, 0),
    agoAmount: state.offloadingRecords.filter(r => r.fuelType === 'AGO').reduce((sum, record) => sum + record.totalAmount, 0)
  };
  ...
  const exportToPDF = () => {
    ...
    const headers = ['Date', 'Time', 'Truck Reg', 'Driver', 'Fuel Type', 'Quantity (L)', 'Rate', 'Total Amount', 'Supplier'];
    ...
    // Add totals
    doc.setFont('helvetica', 'bold');
    doc.text(`Total Quantity: ${formatNumber(totals.totalQuantity)} L`, 14, finalY);
    doc.text(`Total Amount: ${state.companyData.currency} ${formatNumber(totals.totalAmount)}`, 14, finalY + 8);
    doc.text(`PMS: ${formatNumber(totals.pmsQuantity)} L (${state.companyData.currency} ${formatNumber(totals.pmsAmount)})`, 14, finalY + 16);
    doc.text(`AGO: ${formatNumber(totals.agoQuantity)} L (${state.companyData.currency} ${formatNumber(totals.agoAmount)})`, 14, finalY + 24);
  ...
  const exportToExcel = () => {
    ...
    const ws_data = [
      ...
      [],
      ['Date', 'Time', 'Truck Reg', 'Driver Name', 'Fuel Type', 'Quantity (L)', 'Rate', 'Total Amount', 'Supplier', 'Invoice No', 'Remarks'],
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-700">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Total Quantity</span>
            </div>
            <div className="text-2xl font-bold text-blue-600">{formatNumber(totals.totalQuantity)} L</div>
          </div>
          ...
          <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-700">
            ...
            <div className="text-2xl font-bold text-green-600">{state.companyData.currency} {formatNumber(totals.totalAmount)}</div>
          </div>
          
          <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-700">
            <div className="flex items-center gap-2 mb-2">
              <Fuel size={20} className="text-indigo-500" />
              <span className="text-sm font-medium text-yellow-700 dark:text-yellow-300">PMS</span>
            </div>
            <div className="text-lg font-bold text-yellow-600">{formatNumber(totals.pmsQuantity)} L</div>
            <div className="text-sm text-yellow-600">{state.companyData.currency} {formatNumber(totals.pmsAmount)}</div>
          </div>
          
          <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border border-purple-200 dark:border-purple-700">
            <div className="flex items-center gap-2 mb-2">
              <Fuel size={20} className="text-indigo-500" />
              <span className="text-sm font-medium text-purple-700 dark:text-purple-300">AGO</span>
            </div>
            <div className="text-lg font-bold text-purple-600">{formatNumber(totals.agoQuantity)} L</div>
            <div className="text-sm text-purple-600">{state.companyData.currency} {formatNumber(totals.agoAmount)}</div>
          </div>
        </div>
              ...
              {state.offloadingRecords.length === 0 ? (
                <tr>
                  <td colSpan={9} className="text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <p className="text-sm">Add your first offloading record to get started</p>
                  </td>
                </tr>
              ) : (
                state.offloadingRecords
                  ...
                  .map((record) => (
                    <tr key={record.id}>
                      ...
                      <td className="font-mono">{formatNumber(record.quantity)}</td>
                      <td className="font-mono">{state.companyData.currency} {formatNumber(record.rate)}</td>
                      <td className="font-mono font-medium">{state.companyData.currency} {formatNumber(record.totalAmount)}</td>
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">
                {selectedRecord ? 'Edit Offloading Record' : 'New Offloading Record'}
              </h3>
              <button
                onClick={resetForm}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('truckReg', e.target.value.toUpperCase())}
