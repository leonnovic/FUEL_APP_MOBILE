...
function x(t) {
  ...
  return (
    (e.innerHTML = `<span style="font-size:16px;flex-shrink:0">${a[t.type]}</span><span style="flex:1">${t.message}</span><button onclick="this.parentElement.remove()" style="background:none;border:none;color:inherit;cursor:pointer;font-size:14px;padding:0;margin-left:4px;opacity:0.6">&times;</button>`),
    e
  );
}
...
function r(t, e = "info", n = 3e3) {
  const a = m(),
    l = `${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
    d = { id: l, message: t, type: e, duration: n, createdAt: Date.now() };
  (o.push(d), p());
  const i = x(d);
  (a.appendChild(i),
    setTimeout(() => {
      ((i.style.animation = "toastSlideOut 0.3s ease forwards"),
        setTimeout(() => {
          (i.remove(), (o = o.filter(u => u.id !== l)), p());
        }, 300));
    }, n));
}
const g = (t, e) => r(t, "success", e),
  b = (t, e) => r(t, "error", e),
  y = (t, e) => r(t, "warning", e),
  h = (t, e) => r(t, "info", e);
function k(t) {
